...
class Ferry_EDA:
    ...
    def route_analysis(self):
        ...
        route_boardings = df.groupby('route')['boardings'].sum().sort_values(ascending=False)
    ...
    def stop_specific_analysis(self):
        ...
        df['date'] = pd.to_datetime(df['date'])
        df['boardings'] = pd.to_numeric(df['boardings'], errors='coerce')
        stop_boardings = df.groupby('stop')['boardings'].sum().sort_values(ascending=False)
    ...
    def day_of_week(self):
        ...
        df['date'] = pd.to_datetime(df['date'])
        df['boardings'] = pd.to_numeric(df['boardings'], errors='coerce')
        day_of_week_boardings = df.groupby('typeday')['boardings'].mean().sort_values(ascending=False)
        plt.figure(figsize=(8, 5))
